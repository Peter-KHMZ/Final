...
router.post('/', (req, res, next) => {
  ...
  const users = req.app.locals.users;
  const {  FName, LName, Sname, day, month, year, sex, telephone, email, github } = req.body;
  const _id = ObjectID(req.session.passport.user);

  users.updateOne({ _id }, { $set: { FName, LName, Sname, day, month, year, sex, telephone, email, github } }, (err) => {
    if (err) {
      throw err;
    }
